...
async function handleAuthRoutes(req, res, pathname, corsHeaders) {
  ...
  try {
    ...
    if (pathname === '/api/auth/me' && req.method === 'GET') {
      ...
      if (cbUser) return sendJson(res, 200, { ok: true, user: cbUser }, corsHeaders);
      const user = await auth.findUserById(payload.sub);
    ...
    if (pathname === '/api/auth/verify-sms' && req.method === 'POST') {
      ...
      const token = auth.signCloudbaseToken(cbUser);
      return sendJson(res, 200, { ok: true, token, user: auth.publicCloudbaseUser(cbUser) }, corsHeaders);
    }
