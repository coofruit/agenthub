...
const { rebuildAllPublishedAgents } = require('./lib/agent-publish.cjs');
const fileExtract = (() => {
  try {
...
server.listen(PORT, HOST, () => {
  try {
    const rebuilt = rebuildAllPublishedAgents();
    if (rebuilt.length) {
      console.log(`  已重建上架智能体页面: ${rebuilt.join(', ')}`);
    }
  } catch (e) {
    console.warn('[dev-server] 重建上架页面失败:', e.message);
  }
  console.log('');
  ...
  console.log(`  私募股权基金: http://${HOST}:${PORT}/agent-pefund.html`);
